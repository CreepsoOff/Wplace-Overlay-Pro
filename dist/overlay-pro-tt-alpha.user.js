// ==UserScript==
// @name         Overlay Pro TT test
// @namespace    http://tampermonkey.net/
// @version      0.0.12b+3.1.5
...
// @author       ConfiG, shinkonet, Creepso
// @collaborator Creepso, ConfiG
...
(() => {
  ...
  var DEFAULT_FREE_KEYS = WPLACE_FREE.map(([r, g, b2]) => `${r},${g},${b2}`);
  ...
  function tileAndPixelToLonLat(tx, ty, px, py) {
    const x = tx * TILE_SIZE + px;
    const y = ty * TILE_SIZE + py;
    const a2 = 2 * Math.PI * 6378137 / 2;
    const b2 = a2 / TILE_SIZE / 2 ** 10;
    const lon = (x * b2 - a2) / a2 * 180;
    let lat = (a2 - y * b2) / a2 * 180;
    lat = 180 / Math.PI * (2 * Math.atan(Math.exp(lat * Math.PI / 180)) - Math.PI / 2);
    return [lon, lat];
  }
  async function buildOverlayDataForChunkUnified(ov, style) {
    ...
    switch (style) {
      ...
      case "dots": {
        ...
        const canvas = createCanvas(wScaled, hScaled);
        const ctx = canvas.getContext("2d", { willReadFrequently: true });
        ctx.imageSmoothingEnabled = false;
        const temp = createCanvas(wImg, hImg);
        ...
        for (let y = 0; y < hImg; y++) {
          for (let x = 0; x < wImg; x++) {
            ...
            const r = data[idx], g = data[idx + 1], b2 = data[idx + 2], a2 = data[idx + 3];
            if (a2 === 0) continue;
            if (r === 222 && g === 250 && b2 === 206) continue;
            if (cf && cf[`${r},${g},${b2}`] === false) continue;
            ctx.drawImage(img, x, y, 1, 1, x * scale + center, y * scale + center, 1, 1);
          }
        }
        return { url: await canvasToDataURLSafe(canvas), coordinates };
      }
  ...
  async function updateOverlays() {
    ...
    for (const ov of currentOverlays) {
      ...
      try {
        if (existingLayer) {
          map.setPaintProperty(name, "raster-opacity", config.overlayStyle == "full" ? ov.opacity : 1);
          map.moveLayer(name, {
        ...
        const image = await buildOverlayDataForChunkUnified(ov, config.overlayStyle);
        if (existingSource) {
        ...
        if (!existingLayer) {
          map.addLayer({
            ...
            paint: {
              ...
              "raster-opacity": config.overlayStyle == "full" ? ov.opacity : 1
            }
          }, {
            "behind": "pixel-art-layer",
            "above": "pixel-hover",
  ...
  var a = 2 * Math.PI * 6378137 / 2;
  var b = a / TILE_SIZE / 2 ** 10;
  ...
  var overlayTileCache = /* @__PURE__ */ new Map();
  var overlayDirtyTiles = /* @__PURE__ */ new Map();
  function markTileDirty(overlayId, tileKey) {
  ...
  async function updateOverlayColorStats(ov) {
    ...
    const neededTiles = /* @__PURE__ */ new Set();
    for (let y = 0; y < hImg; y++) {
      for (let x = 0; x < wImg; x++) {
        const idx = (y * wImg + x) * 4;
        const a2 = data[idx + 3];
    ...
    if (base) {
      ...
      const tileCache = overlayTileCache.get(ov.id) || /* @__PURE__ */ new Map();
      const dirty = overlayDirtyTiles.get(ov.id) || /* @__PURE__ */ new Set();
      const tilePromises = Array.from(neededTiles).map(async (key) => {
        ...
        try {
          ...
          const tileCanvas = createCanvas(tileImg.width, tileImg.height);
      ...
      if (dirty.size > 0) overlayDirtyTiles.set(ov.id, dirty);
      else overlayDirtyTiles.delete(ov.id);
      ...
      for (let y = 0; y < hImg; y++) {
        for (let x = 0; x < wImg; x++) {
          ...
          if (tdata) {
            ...
            if (r === tr && g === tg && b2 === tb) mismatch = false;
          }
  ...
  function weightedNearest(r, g, b2, palette) {
  ...
  function getActivePalette() {
    ...
    cc.selectedFree.forEach((k) => {
      const [r, g, b2] = k.split(",").map((n) => parseInt(n, 10));
      if (Number.isFinite(r)) arr.push([r, g, b2]);
    });
  ...
  function processImage() {
    ...
    for (let i = 0; i < src.length; i += 4) {
      const r = src[i], g = src[i + 1], b2 = src[i + 2], a2 = src[i + 3];
      if (a2 === 0) {
        out[i] = 0;
        out[i + 1] = 0;
        out[i + 2] = 0;
      ...
      const [nr, ng, nb] = palette.length ? weightedNearest(r, g, b2, palette) : [r, g, b2];
      out[i] = nr;
      out[i + 1] = ng;
      out[i + 2] = nb;
      out[i + 3] = 255;
  ...
  function renderPaletteGrid() {
    ...
    for (const [r, g, b2] of WPLACE_FREE) {
      const key = `${r},${g},${b2}`;
    ...
    for (const [r, g, b2] of WPLACE_PAID) {
      const key = `${r},${g},${b2}`;
      const cell = document.createElement("div");
      cell.className = "op-cc-cell";
      cell.style.background = `rgb(${r},${g},${b2})`;
  ...
  function isAllPaidActive() {
    const allPaidKeys = WPLACE_PAID.map(([r, g, b2]) => `${r},${g},${b2}`);
  ...
  function setAllActive(type, active) {
    if (type === "free") {
      ...
      const keys = WPLACE_PAID.map(([r, g, b2]) => `${r},${g},${b2}`);
  ...
  async function reconstructViaGrid(img, origW, origH, offx, offy, gapX, gapY) {
    ...
    for (let ry = 0; ry < rows; ry++) {
      for (let rx = 0; rx < cols; rx++) {
        ...
        const b2 = srcData[si + 2];
        const a2 = srcData[si + 3];
  ...
  function rebuildOverlayListUI() {
    ...
    `;
      const [radio, checkbox, nameDiv, jumpBtn, trashBtn] = item.children;
  ...
  async function importOverlayFromJSON(jsonText) {
    ...
    for (const item of arr) {
      const override = shouldOverride === false ? void 0 : config.overlays.find((x) => x.name.toLowerCase() === item.name.toLowerCase());
      ...
      if (override && shouldOverride) {
        ...
        override.pixelUrl = item.pixelUrl !== void 0 ? item.pixelUrl : override.pixelUrl;
        override.offsetX = item.offsetX !== void 0 ? item.offsetX : override.offsetX;
        override.offsetY = item.offsetY !== void 0 ? item.offsetY : override.offsetY;
        override.opacity = item.opacity !== void 0 ? item.opacity : override.opacity;
        imported++;
      } else {
  ...
  function exportActiveOverlayToClipboard() {
    ...
    const payload = {
      ...
      offsetX: ov.offsetX == 0 ? void 0 : ov.offsetX,
      offsetY: ov.offsetY == 0 ? void 0 : ov.offsetY,
      opacity: ov.opacity == 0.7 ? void 0 : ov.opacity
    };
  ...
  function addEventListeners(panel) {
    ...
    $("op-import-overlay").addEventListener("click", async () => {
      const text = prompt("Paste overlay JSON (single or array) or link:");
      if (!text) return;
      await importOverlayFromJSON(text);
    });
    $("op-export-overlay").addEventListener("click", () => exportActiveOverlayToClipboard());
    $("op-collapse-stats").addEventListener("click", () => {
      config.collapseStats = !config.collapseStats;
      saveConfig(["collapseStats"]);
      updateUI();
    });
    ...
    $("op-download-overlay").addEventListener("click", () => {
      ...
      a2.href = ov.imageBase64;
      a2.download = `${(ov.name || "overlay").replace(/[^\w.-]+/g, "_")}.png`;
  ...
  function rebuildColorFilterUI() {
    ...
    const filter = ov.colorFilter || {};
    const entries = Object.entries(stats).sort((a2, b2) => b2[1].total - a2[1].total);
    let total = 0;
    let remaining = 0;
  ...
  function updateThemeToggle() {
    ...
    themeToggle.textContent = config.theme === "dark" ? "\u2600\uFE0F" : "\u{1F319}";
  }
  ...
  function updateUI() {
    ...
    header.style = collapsed ? "border-bottom: none;" : void 0;
    ...
    if (me.data) {
      ...
      const percent = Math.floor((me.data.level - level) * 100);
      ...
      if (smallStats) {
        smallStats.textContent = `(${me.data.droplets}\u{1F4A7}| ${pixels - forNextLevel}/${pixels - forCurrentLevel}/${forNextLevel - forCurrentLevel})`;
      }
    }
    if (smallStats) {
    ...
    for (const layering of ["Behind", "Above", "Top"]) {
      ...
      button.addEventListener("click", async () => {
        config.overlayLayering = layering.toLowerCase();
  ...
  function attachHook() {
      ...
      page.Promise = class extends page.PromiseOrig {
        constructor(executor) {
          ...
          if (!executor.toString().includes("maps.wplace.live"))
            return;
          this.then(async (x) => await mapCreated(x));
          page.Promise = page.PromiseOrig;
          page.PromiseOrig = void 0;
        }
      };
    }
    if (hookInstalled)
      return;
    const hookedFetch = (originalFetch) => async (input, init) => {
    ...
    window.fetch = hookedFetch(window.fetch);
    hookInstalled = true;
  }
